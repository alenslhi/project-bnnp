...
@section('title', 'Tambah Zona Kerawanan')
@section('page-title', 'Tambah Data Zona')
...
<div class="max-w-3xl">
    ...
    <div class="rounded-3xl border border-white/10 bg-surface-900/60 backdrop-blur-xl p-6 sm:p-8 shadow-xl">
        <form action="{{ route('admin.zona.store') }}" method="POST" class="space-y-6">
            @csrf
            ...
            <div>
                ...
                <input type="text" name="nama_wilayah" id="nama_wilayah" value="{{ old('nama_wilayah') }}" required
                       placeholder="Contoh: Kota Palu, Kab. Donggala, Kab. Poso"
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select name="status_zona" id="status_zona" required
                        ...
                        <option value="">Pilih Status Zona</option>
                        <option value="merah" {{ old('status_zona') === 'merah' ? 'selected' : '' }}>🔴 Merah (Bahaya / Rawan Tinggi)</option>
                        <option value="kuning" {{ old('status_zona') === 'kuning' ? 'selected' : '' }}>🟡 Kuning (Waspada / Rawan Sedang)</option>
                        <option value="hijau" {{ old('status_zona') === 'hijau' ? 'selected' : '' }}>🟢 Hijau (Aman / Rawan Rendah)</option>
                    </select>
                    @error('status_zona')
                        <p class="mt-1 text-xs text-danger-400">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input type="number" name="jumlah_kasus" id="jumlah_kasus" value="{{ old('jumlah_kasus', 0) }}" min="0" required
            ...
            <div>
                ...
                <textarea name="deskripsi" id="deskripsi" rows="3"
                          placeholder="Catatan intelijen, jenis peredaran dominan, atau titik penindakan..."
                          class="w-full rounded-xl border border-white/10 bg-surface-950/70 px-4 py-3 text-sm text-white placeholder-surface-500 focus:border-primary-500 focus:outline-none focus:ring-2 focus:ring-primary-500/20 transition">{{ old('deskripsi') }}</textarea>
                @error('deskripsi')
                    <p class="mt-1 text-xs text-danger-400">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <textarea name="koordinat_geojson" id="koordinat_geojson" rows="4"
                          placeholder='{"type": "Feature", "geometry": {"type": "Point", "coordinates": [119.8707, -0.8917]}}'
                          class="font-mono text-xs w-full rounded-xl border border-white/10 bg-surface-950/70 px-4 py-3 text-white placeholder-surface-600 focus:border-primary-500 focus:outline-none focus:ring-2 focus:ring-primary-500/20 transition">{{ old('koordinat_geojson') }}</textarea>
                <p class="mt-1.5 text-[11px] text-surface-500">Kosongkan jika ingin menggunakan koordinat default peta provinsi Sulawesi Tengah.</p>
